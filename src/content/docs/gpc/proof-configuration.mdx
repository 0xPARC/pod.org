...
You can use GPC to make proofs about PODs, but how do you specify what you want to prove? You can learn more about the proving flow on the [Proving and Verification](./proving-and-verifying) page.  This page covers the full configurability of GPCs, and all the things you can prove with them.

## Configuring a Proof

When you generate a proof, you say what to prove using two arguments. [Proof Configuration](https://docs.pcd.team/types/_pcd_gpc.GPCProofConfig.html) specifies what you're proving in the form of constraints. It also gives names to the inputs and specifies which parts of those inputs should be revealed. [Proof Inputs](https://docs.pcd.team/types/_pcd_gpc.GPCProofInputs.html) uses the names assigned by the config to provide the data which satisfies the constraints.

A proof config is often fixed by the app use case, and reused for many proofs. The config might be part of a request from the verifier which triggers the creation of a proof.  In contrast, at least some of the inputs are likely to vary with each proof. It might be up to the end user to select the inputs which will make the proof valid.

Each of the constraints of a proof correspond to a module in the [ZK circuit](https://docs.circom.io/background/background/) used to generate the proof. For instance, each POD mentioned in the configuration needs an Object module to verify its signature.  Each entry mentioned in the configuration needs an Entry module to verify its inclusion in the POD's Merkle tree.

The [GPC compiler](./proving-and-verifying#overall-flow) translates the application's configuration and inputs into the numeric inputs required by the modules. It also picks a circuit from the available family with enough modules to accept those inputs. More modules means a larger circuit and a more expensive proving process (more time, more memory). See the [artifacts](./artifacts) page for more about the modular circuit family.
...
This configuration implies the need to include four inputs, which are given names in the configuration.  Some of these names are cryptographically important while others are developer choice:
...
- `SemaphoreV4` identifies the type of private key the owner must provide to prove their ownership of the POD.
  - The name `SemaphoreV4` corresponds to one of the owner identity formats supported by the library.


## Configuration Structure

The GPCProofConfig type is structured around the different categories of constraints you can use in a proof. See the [API Reference](https://docs.pcd.team/types/_pcd_gpc.GPCProofConfig.html) for full details. The example below shows how each category of constraints fits into the configuration.  Each comment corresponds to one of the [constraint](#supported-constraints) sections below.
...
- **Entry constraints** apply to a single entry of a single POD. The entry configuration always specifies whether the entry value should be revealed, then usually contains additional [constraints](#entry-constraints) on the entry.
...
- **Global constraints** apply to all of the input PODs together. This includes [tuples](#tuples) which group together multiple entries as a single value.  There are also some [global constraint](#global-constraints) which control the behavior of the overall proof.
- **Other Inputs** which don't directly match the config structure are also described [below](#other-inputs)

### Entry Constraints

The named entries of PODs are the primary source of data to constrain in your proofs, and the most types of constraints, most of them optional.

<Aside type="note" title="Presence or Absence">
By mentioning an entry in your configuration you are implicitly proving that such an entry exists in the POD.  Even if the value is not revealed, the presence an entry by that name is revealed. If that's all you want to prove, you can configure an entry with `isRevealed: false` and no other constraints. You'll need to do this if you want to refer to the entry by name in another entry or tuple.

Any entries not mentioned in the configuration may or may not exist in the POD since nothing is being proven about them. You might trust the issuer to always follow a schema containing certain entries, but nothing stops anyone from issuing a POD containing some but not all of a schema.

It isn't possible to directly prove an entry does not exist in a POD. A possible workaround for that is to issue PODs with a the value `null` and reveal that entry in your configuration.
</Aside>

The following constraints can be applied to any type of value, because they operate on the value hash:
...
- **Ownership** ([isOwnerID](https://docs.pcd.team/types/_pcd_gpc.GPCProofEntryConfig.html)) allows the prover to assert ownership of a POD.  They do this by constraining this entry to be equal to the public identifier which corresponds to the private identifier (private key) presented as one of the proof inputs.  The configuration of this constraint doesn't specify the owner's ID, but the name of the identity protocol to use (most often `SemaphoreV4`).


The following constraints can be applied only to numeric values which fit in a 64-bit signed integer:
...
This applies to value constraints like `isMember` and also to numeric constraints like `greaterThan`.  The value `{boolean: true}` is a member of a list containing `{int: 1}`.  Similarly you can constrain that a value of `{date: "1999-01-01"}` is greather than `{cryptographic: 0}`.  Dates are represented as numbers using millis since the epoch (following the JavaScript Date type).
</Aside>
...
- [signerPublicKey](https://docs.pcd.team/types/_pcd_gpc.GPCProofObjectConfig.html) is public key of the signer of the POD. This is revealed by default, since you shouldn't trust a POD if you don't know who signed it.  You can choose to make this private, though you should probably only do so if you are constraining it in some other way, such as using list membership or a tuple.
- [contentID](https://docs.pcd.team/types/_pcd_gpc.GPCProofObjectConfig.html) is the content ID of the POD, i.e. the root of its Merkle tree. This is private by default, since revealing it reveals some information about the contents of the POD (potentially leading to offline guessing attacks). Comparing an entry of one POD to the content ID of another POD is one way for PODs to refer to or "contain" other PODs.

Virtual entries can be constrained using a subset of the [entry constraints](#entry-constraints) described above.  Specifically you can specify Visibility, 


<Aside type="note" title="POD Validity Checks">
The virtual entries of any given POD are tied to the verification of the structure of the POD. By including a POD in a proof, you are implicitly constraining it to have a valid signature, and constraining each of the named entries to be members of the Merkle tree represented by the POD's Content ID. These things are always verified by a GPC proof even if you don't mention the virtual entries in your configuration.
</Aside>
...
For example, in the [example](#a-real-world-example) above, rather than separately constraining the signer and event ID, you form them into a pair so that each signer is only allowed to issue tickets to their own events.  You'd then declare a tuple like this:
...
- **Watermark**: This is an extra value included in the proof in a cryptographically secure way. The watermark can be any [PODValue]() such as a string or number, and can be used to include any extra app-specific data in the proof. Watermarks are often used to hold a unique nonce or timestamp, or to tie a proof to a larger message to avoid replay attacks. See the page on [Verifying Securely](./verification) for more suggestions.
- **Owner**: Ownership of a POD can be established by proving that you hold the private key which corresponds to an identity mentioned in the POD. See the [Identity and Ownership](./identity-ownership) page for more info.
...
1. Anything not mentioned in the configuration isn't being proven and thus may not be true. For example, any entry not named in your proof may or may not be present in the POD, depending on the issuer's behavior. A value you expect to be a number might actually be a string, unless you constrained it in some way.
2. PODs are freely copyable. All the PODs mentioned in a proof could be copies of a single POD, unless you use the [`uniquePODs` constraint](#global-constraints).
3. The signers of each POD (`signerPublicKey`) should always be checked in some way (see [Verifying Securely](#verification)). Anyone an sign a POD with a random key, but it's trusting the signer which makes a POD meaningful. Signers can be checked in several ways:
...
4. POD value types are hints, checked by the library but not by the cryptographic proof. Keep in mind the [equivalency classes](/pod/values) of values which can be identical across types. For instance, even if the expected schema of a POD specifies an `int` value, you can't be sure it's not a huge number until you check it with an `inRange` constraint.
5. You can prove the presence of a named entry without revealing it, but you can't prove the absence of a named entry without revealing the whole POD. If you're issuing PODs with optional entries, consider setting them to `null` if you think provers need to constrain them in an "unused" state.

## Additional Resources

- For detailed documentation of all parts of configuration, see the API reference for [GPCProofConfig](https://docs.pcd.team/types/_pcd_gpc.GPCProofConfig.html) and [GPCProofInputs](https://docs.pcd.team/types/_pcd_gpc.GPCProofInputs.html).
