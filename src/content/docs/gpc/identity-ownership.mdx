...
[Zupoll](https://zupoll.org/) (which uses Semaphore group proofs pre-dating GPCs) avoids this attack by using a snapshot of voting group members at the time that a poll begins. A user with a series of identities created by account resets would still have only one identity in the group. A POD-based app could use an [list membership constraint](./proof-configuration#entry-constraints) to constrain the user's identity (or their ticket ID) to be part of a similar snapshot. Alternative approaches might reveal the issuance time of the ticket and accept only recently-issued tickets, or use a [list non-membership constraint](./proof-configuration#entry-constraints) to reject tickets which have been revoked to be re-issued.
